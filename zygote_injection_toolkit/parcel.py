...
class ParcelType:
    ...
    def _decode_variable(self, data: bytes) -> tuple[int, Any]:
        length = self.length
        if length is None:
            raise ValueError('cannot decode a type of variable length without variable decode method')
        else:
...
class ParcelString(ParcelType):
    ...
    def _decode_variable(self, data: bytes) -> tuple[int, str]:
        string_length_bytes = data[0:4]
        if len(string_length_bytes) < 4:
            raise ValueError('not enough data to parse string length')
        string_length = int.from_bytes(string_length_bytes, 'little')
        if string_length == -1:
            return (4, None)
        total_string_bytes = string_length * 2
        total_bytes_consumed = 4 + total_string_bytes + 2
        if len(data) < total_bytes_consumed:
            raise ValueError('not enough data to parse string content')
        string_data = data[4:4 + total_string_bytes]
        string = string_data.decode('utf-16le')
        null_terminator = data[4 + total_string_bytes:total_bytes_consumed]
        if null_terminator != b'\x00\x00':
            warn('String in parcel did not end with expected null terminator')
        return (total_bytes_consumed, string)
...
class ServiceFunction:
    ...
    def __init__(self, code: int, schema: Optional[SchemaType], return_type: Optional[ParcelType], name: Optional[str]=None) -> None:
        self.code = code
        self.schema = schema
    ...
    def parse_arguments(self, arguments: list[Any]) -> list[str]:
        ...
        if len(self.schema) != len(arguments):
            raise ValueError(f"service function takes {len(self.schema):d} arguments but {len(arguments)} {('was' if len(arguments) == 1 else 'were')} given")
        encoded_arguments = []
...
def parse_aidl_type(aidl_type: aidl.tree.Type) -> Optional[ParcelType]:
    if aidl_type is None:
        return ParcelVoid
    BASIC_TYPES = {'int': ParcelInt32, 'long': ParcelInt64, 'boolean': ParcelBool, 'float': ..., 'double': ...}
    if aidl_type.dimensions:
        ...
        if aidl_type.name == 'String':
            return ParcelString
    if isinstance(aidl_type, aidl.tree.BasicType) and aidl_type.name == 'byte' and (aidl_type.dimensions == [None]):
        return ParcelVoid
...
def parse_aidl_interface(aidl_definition: aidl.ast.Node, interface_name: str) -> InterfaceType:
    ...
    for function, code in zip(aidl_interface.body, range(len(aidl_interface.body))):
        ...
        result[function.name] = ServiceFunction(code, schema, return_type, function.name)
    return result