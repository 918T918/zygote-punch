...
def dump_info(serial: str=None, port: int=1234):
    ...
    try:
        ...
        properties = {'Model': s1.getprop('ro.product.model'), 'Manufacturer': s1.getprop('ro.product.manufacturer'), 'Android Version': s1.getprop('ro.build.version.release'), 'Security Patch': s1.getprop('ro.build.version.security_patch'), 'Build ID': s1.getprop('ro.build.display.id'), 'Kernel': s1.shell_execute('uname -r')['stdout'].strip(), 'SELinux Status': s1.shell_execute('getenforce')['stdout'].strip()}
