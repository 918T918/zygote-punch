...
class AdbHelper:
        ...
        if len(lines) > 1:
            ...
            for line in lines[1:]:
                ...
                if len(parts) >= 2:
                    ...
                    conn_type = "USB" if not (":" in serial and re.match(r"\d+\.\d+\.\d+\.\d+", serial)) else "TCP/IP"
    ...
    def get_installed_apps(serial: Optional[str] = None) -> List[Dict[str, str]]:
        ...
        regex = re.compile(r"package:(?P<path>.*?)=(?P<pkg>[\w\.]+)\s+uid:(?P<uid>\d+)")
    ...
    def get_target_apps(serial: Optional[str] = None) -> List[Dict[str, str]]:
        ...
        for app in all_apps:
            ...
            if is_priv or is_target_uid:
                ...
                if is_priv: app['type'].append("priv-app")

                if uid == 1000: app['type'].append("system(1000)")

                if uid == 1001: app['type'].append("radio(1001)")
    ...
    def get_detailed_package_info(package_name: str, serial: Optional[str] = None) -> Dict[str, Optional[str]]:
        ...
        seinfo_match = re.search(r"seInfo=([\w:]+)", output)
