...
def full_interactive_mode(ctx):
    ...
    while True:
        ...
        try:
            ...
            if ctx.transport_id: info_lines.append(f"Transport ID: [cyan]{ctx.transport_id}[/]")

            if ctx.usb: info_lines.append("Mode: [magenta]USB Force[/]")

            elif ctx.tcpip: info_lines.append("Mode: [magenta]TCP/IP Force[/]")
            ...
            console.print("3. [highlight]App Management[/] (Backup, Restore, Permissions, Manage)")

            console.print("4. [highlight]Interactive Shell[/]")

            console.print("5. [highlight]SELinux Analysis[/]")

            console.print("6. [highlight]Switch/List Devices[/]")
            ...
            if choice == "1":
                ...
                if sub_choice == "Unlock OEM": stage2.exploit_stage2()

                elif sub_choice == "Disable Hidden API": stage2.toggle_hidden_api()

            elif choice == "2":

                sub_choice = Prompt.ask("Forensics Action", choices=["Telephony (IMEI/IMSI)", "Accounts", "Device Info Dump", "Back"])

                if sub_choice == "Telephony (IMEI/IMSI)":
                    ...
                    for a in accs: table.add_row(a['name'], a['type'])

                    console.print(table)

                    Prompt.ask("\nPress Enter to continue")

                elif sub_choice == "Device Info Dump":
                ...
                sub_choice = Prompt.ask("App Action", choices=["Backup", "Restore", "Grant Permission", "Disable/Stop App", "Back"])

                if sub_choice == "Back": continue

                pkg = Prompt.ask("Package Name")

                if sub_choice == "Backup": stage2.backup_data(pkg, Prompt.ask("Output File", default=f"{pkg}.tar.gz"))

                elif sub_choice == "Restore": stage2.restore_data(pkg, Prompt.ask("Input File"))

                elif sub_choice == "Grant Permission": stage2.grant_permission(pkg, Prompt.ask("Permission"))

                elif sub_choice == "Disable/Stop App":
...
def run_toolkit(args):
    ...
    if not args.no_kill: kill_existing_processes()
    ...
    if args.action == "check":
        ...
        except Exception as e: console.print(f"[error][-] NOT VULNERABLE: {e}[/]")
    ...
    with RemoteShellSession(port=args.port) as session:
        ...
        if args.action == "shell":
            ...
            if not args.package or not args.output: error_exit("--package and --output required for backup")

            stage2.backup_data(args.package, args.output)

        elif args.action == "restore":

            if not args.package or not args.output: error_exit("--package and --output required for restore")

            stage2.restore_data(args.package, args.output)

        elif args.action == "grant":

            if not args.package or not args.permission: error_exit("--package and --permission required")

            stage2.grant_permission(args.package, args.permission)

        elif args.action == "hidden-api": stage2.toggle_hidden_api()

        elif args.action == "package":

            if not args.package or not args.pkg_action: error_exit("--package and --pkg-action required")

            stage2.manage_package(args.package, args.pkg_action)

        elif args.action == "exploit": stage2.exploit_stage2()
